...
router.get("/", async (req, res) => {
  ...
  const winners = await db
    .select({
      ...
      poolTitle: poolsTable.title,
      poolCurrentMembers: poolsTable.currentMembers,
      poolSoldTickets: poolsTable.soldTickets,
  ...
  const enriched = await Promise.all(
    winners.map(async (w) => {
      ...
      poolNumber: w.poolId,
      poolCurrentMembers: w.poolCurrentMembers,
      poolSoldTickets: w.poolSoldTickets,
      poolTotalTickets: w.poolTotalTickets,
      poolMaxUsers: w.poolMaxUsers,
      userId: w.userId,
      userName: privacyDisplayName(w.userName),
