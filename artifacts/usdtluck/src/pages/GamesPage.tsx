  ...
  return (
    <>
      ...
      <div className="md:hidden" style={{ padding: "12px var(--page-px) 0" }}>
        <p style={{ fontSize: 14, fontWeight: 600, margin: "12px 0 10px", color: "var(--text-white)" }}>
          Play and win
        </p>

        {stateLoading ? (
          <p style={{ padding: "20px 0", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>
            Loading…
          </p>
        ) : stateError ? (
          <div
          ...
          >
            <p style={{ fontSize: 13, fontWeight: 600, color: "var(--text-white)" }}>Games unavailable</p>
            <p style={{ marginTop: 6, fontSize: 11, color: "var(--text-muted)" }}>
              {gameState?.reason === "GAMES_PREMIUM_REQUIRED"
                ? "Upgrade your pool VIP tier to unlock games."
                : "The arcade is temporarily unavailable."}
            </p>
            <div style={{ marginTop: 10 }}>
              <Link href="/pools" className="inline-flex">
                <span
                  role="button"
                  style={{
                    height: 40,
          ...
          <>
            ...
            >
              {[
                { href: "/games/spin-wheel", icon: "🎡", name: "Risk Wheel", range: "1.2x — 3x" },
                { href: "/games/mystery-box", icon: "📦", name: "Treasure Hunt", range: "Up to ~6.5x" },
                { href: "/games/scratch-card", icon: "🔢", name: "Lucky Numbers", range: "Up to 10x" },
                { href: "/games/hi-lo", icon: "🃏", name: "Hi‑Lo Cards", range: "Up to 5x" },
              ].map((g) => (
                <Link key={g.href} href={g.href} onClick={() => play("tap")}>
                  <div
                    role="button"
                  ...
                  >
                    <div style={{ fontSize: 32, marginBottom: 6 }} aria-hidden>
                      {g.icon}
                    </div>
                    <p style={{ fontSize: 13, fontWeight: 600, color: "var(--text-white)" }}>{g.name}</p>
                    <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{g.range}</p>
                    <span
                      style={{
            ...
            <div style={{ marginTop: 14 }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-white)", margin: "12px 0 10px" }}>
                Recent wins
              </p>
              {wins.length === 0 ? (
                <p style={{ fontSize: 13, color: "var(--text-muted)" }}>No recent wins yet.</p>
              ) : (
                <div>
                  {wins.slice(0, 10).map((w: any, idx: number) => (
                    <div
                      key={`${w.id ?? idx}`}
                      style={{
                        display: "flex",
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                        <span style={{ width: 24, textAlign: "center", fontSize: 16 }} aria-hidden>
                          {idx % 3 === 0 ? "🥇" : idx % 3 === 1 ? "🥈" : "🥉"}
                        </span>
                        <div style={{ minWidth: 0 }}>
                          <p style={{ fontSize: 13, fontWeight: 500, color: "var(--text-white)" }}>
                            {String(w.userName ?? "Winner")}
                          </p>
                          <p style={{ fontSize: 11, color: "var(--text-dim)" }}>
                            {String(w.gameName ?? "Game")} · {String(w.timeAgo ?? "")}
                          </p>
                        </div>
                      </div>
      ...
      {/* Desktop / existing games hub */}
      <div className="hidden md:block sp-ambient-bg relative min-h-[75vh] w-full max-w-5xl mx-auto px-4 sm:px-6 py-8">
